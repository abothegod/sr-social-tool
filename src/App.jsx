...
const BRAND_CONTEXT = `
BUSINESS DETAILS — weave naturally into content where relevant:
- Name: Shroom Room | Website: shroomroom.ca
- Products: Dry mushrooms, capsules, chocolates, gummies, weed
- Delivery: Local GTA (Greater Toronto Area) delivery. Ships across all of Canada.
- Pricing:
    • Orders UNDER $100 → $20 flat delivery fee
    • Orders $100 or MORE → FREE mystery gift included
    • Orders $200 or MORE → FREE product of customer's choice
- New customer promo: code NEWB20 for 20% off first order
- Brand voice: Trippy, neon psychedelic, mystical, community-driven, professional
`;
...
function CopyBtn({ text, color }) {
  ...
  return (
    <button onClick={go} style={{
      background: done ? `${color}33` : "#1a1a2a", border: `1px solid ${done ? color : "#333"}`,
      borderRadius: 4, color: done ? color : "#aaa", padding: "5px 14px",
...
function TabBtn({ active, color, glow, onClick, children }) {
  return (
    <button onClick={onClick} style={{
      ...
      padding: "9px 16px", cursor: "pointer", fontFamily: "monospace",
  ...
  return (
    <div style={{ textAlign: "center", padding: "60px 20px", color: "#444" }}>
      ...
      <div style={{ fontSize: 13, fontFamily: "monospace", lineHeight: 1.8, color: "#555" }}>
        Configure settings on the left<br />and hit <span style={{ color }}>Generate</span>
      </div>
    </div>
  );
}
    ...
    <div style={{ background: "#161616", border: "1px solid #2a2a2a", borderRadius: 14, overflow: "hidden", boxShadow: `0 4px 40px ${ac.color}15` }}>
      ...
      <div style={{ width: "100%", aspectRatio: "1/1", background: "linear-gradient(135deg,#0d0020,#000d1a,#001a0d)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", textAlign: "center", padding: "0 24px" }}>
          <div style={{ color: ac.color, fontSize: 10, fontFamily: "monospace", letterSpacing: "0.35em", marginBottom: 10 }}>SHROOMROOM.CA</div>
          {result?.seoTitle
    ...
    <div style={{ background: "#0f0f1a", border: "1px solid #252535", borderRadius: 10, overflow: "hidden", fontFamily: "sans-serif" }}>
      <div style={{ background: "linear-gradient(135deg,#0d0020,#000d1a)", padding: "28px 28px 20px", borderBottom: "1px solid #1a1a2a", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ color: "#fff", fontSize: 22, fontWeight: 700, lineHeight: 1.3 }}>{result.emailSubject}</div>
          <div style={{ color: "#888", fontSize: 12, marginTop: 8 }}>{result.preheader}</div>
        </div>
      </div>
      <div style={{ padding: "24px 28px" }}>
        <div style={{ color: "#ddd", fontSize: 13, lineHeight: 1.85, whiteSpace: "pre-wrap" }}>{result.emailBody?.slice(0, 400)}…</div>
        {result.ctaText && (
          <div style={{ textAlign: "center", marginTop: 24 }}>
            <div style={{ display: "inline-block", background: ac.color, color: "#000", padding: "12px 32px", borderRadius: 6, fontWeight: 700, fontSize: 14 }}>{result.ctaText}</div>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ background: "#0f0f1a", border: "1px solid #252535", borderRadius: 10, overflow: "hidden", fontFamily: "sans-serif" }}>
      ...
      <div style={{ padding: "24px 32px" }}>
        <div style={{ color: "#ddd", fontSize: 13, lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{result.blogIntro?.slice(0, 450)}…</div>
      </div>
    </div>
  );
}
...
function buildPrompt(contentType, aura, design, topic) {
  ...
  const imagePrompts = `"imagePrompts": [
    { "label": "Hero Banner", "prompt": "Bold presentation graphic: dark neon background, large centered text overlay in glowing ${ac.color} neon font, dramatic ${dc.label} psychedelic mushroom illustration, particle effects, professional Instagram post, 1080x1080px" },
    { "label": "Carousel Slide", "prompt": "Split-panel: left shows glowing neon ${dc.label} mushroom, right has dark background with bold headline in ${ac.color}, subheadline white, shroomroom.ca logo bottom, editorial magazine style" },
    { "label": "Story Format", "prompt": "Vertical 9:16 graphic: full bleed ${dc.label} neon mushroom, bold white text top third, dark overlay for readability, swirl effects, shroomroom.ca watermark" },
    { "label": "Minimalist Square", "prompt": "Dark minimalist: centered oversized glowing ${dc.label} mushroom, neon headline in ${ac.color}, subtle grid background, clean white body text, shroomroom.ca footer, premium brand ad feel" }
  ]`;

  if (contentType === "instagram") {
    return {
      sys: `You are an Instagram content creator and SEO strategist for Shroom Room (shroomroom.ca), a Canadian psilocybin & magic mushroom dispensary.\n${BRAND_CONTEXT}\nReturn ONLY valid JSON. No markdown, no backticks. Start with { end with }.\n{\n  "seoTitle": "max 60 char SEO title",\n  "caption": "200-260 word emoji-rich caption with hook, value, CTA mentioning shroomroom.ca and relevant delivery perks",\n  "hashtags": ["28 hashtag strings no # symbol, include Toronto/GTA/Canada tags"],\n  "altText": "1-2 sentence accessibility description",\n  "seoKeywords": ["8 SEO keyword strings"],\n  ${imagePrompts}\n}`,
      usr: `Aura: ${aura} — ${ac.desc}\nDesign: ${dc.label}\nTopic: ${topic}\nTone: ${aura === "promotion" ? "urgent, exciting, highlight free gifts and delivery perks" : aura === "education" ? "informative, scientific yet psychedelic" : "mystical, poetic, spiritual"}`
    };
  }
  if (contentType === "blog") {
    return {
      sys: `You are an SEO content writer for Shroom Room (shroomroom.ca), a Canadian psilocybin & magic mushroom dispensary.\n${BRAND_CONTEXT}\nReturn ONLY valid JSON. No markdown, no backticks. Start with { end with }.\n{\n  "blogTitle": "SEO-optimized blog title",\n  "blogCategory": "category e.g. Education, Wellness",\n  "metaDescription": "150-160 char SEO meta description",\n  "readTime": "e.g. 6 min read",\n  "blogIntro": "150-word intro with hook",\n  "blogOutline": ["6-8 H2 section headings"],\n  "blogBody": "500-600 word article with CTA for shroomroom.ca",\n  "seoKeywords": ["10 SEO keyword strings including Toronto/GTA/Canada"],\n  "internalLinks": ["3-4 internal link anchor texts"],\n  "metaTags": ["8 meta tag strings"],\n  ${imagePrompts}\n}`,
      usr: `Aura: ${aura}\nDesign: ${dc.label}\nTopic: ${topic}\nTone: ${aura === "promotion" ? "benefit-focused, weave in delivery perks as conversion hooks" : aura === "education" ? "authoritative, research-backed" : "spiritual, experiential"}`
    };
  }
  if (contentType === "email") {
    return {
      sys: `You are an email marketing strategist for Shroom Room (shroomroom.ca), a Canadian psilocybin & magic mushroom dispensary.\n${BRAND_CONTEXT}\nReturn ONLY valid JSON. No markdown, no backticks. Start with { end with }.\n{\n  "emailSubject": "subject line under 50 chars",\n  "preheader": "preview text under 90 chars",\n  "emailBody": "350-450 word email with greeting, value, mention delivery perks and NEWB20 promo where relevant, strong CTA to shroomroom.ca",\n  "ctaText": "CTA button text under 30 chars",\n  "ctaUrl": "shroomroom.ca/shop",\n  "subjectLineVariants": ["3 alternative subject lines"],\n  "segmentTip": "1-sentence segment tip",\n  "seoKeywords": ["6 relevant keywords"],\n  ${imagePrompts}\n}`,
      usr: `Aura: ${aura}\nDesign: ${dc.label}\nTopic: ${topic}\nTone: ${aura === "promotion" ? "urgent, exclusive, lead with free gift tiers and NEWB20" : aura === "education" ? "helpful, expert" : "warm, spiritual, community-first"}`
    };
  }
}
...
export default function ShroomRoomStudio() {
  ...
  useEffect(() => { setTab("preview"); setResult(null); }, [contentType]);
  useEffect(() => {
    try { const s = localStorage.getItem("sr_saved_v3"); if (s) setSaved(JSON.parse(s)); } catch {}
  }, []);

  const persist = (posts) => {
    setSaved(posts);
    try { localStorage.setItem("sr_saved_v3", JSON.stringify(posts)); } catch {}
  };
  ...
  const tabSets = {
    ...
    email:     [["preview","📧 Preview"],["copy","📝 Copy"],["variants","💡 Variants"],["images","🎨 Image Prompts"]],
  };

  const fullIGText = result ? `${result.caption}\n\n${"#" + result.hashtags?.join(" #")}` : "";

  const ExportCard = ({ items }) => (
    <Card>
      <SLabel>◈  COPY & EXPORT</SLabel>
      {items.filter(i => i.text).map(({ label, text }, i, arr) => (
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#07070f", fontFamily: "'Space Mono',monospace" }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "24px 20px", position: "relative" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <p style={{ color: "#666", fontSize: 12, letterSpacing: "0.18em", marginTop: 8, fontFamily: "monospace" }}>Instagram · Blog · Email · SEO · Image Prompts</p>
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 20 }}>
              {[["create","⬡  Create"],["saved",`⊹  Saved (${saved.length})`]].map(([k,l]) => (
                <button key={k} onClick={() => setMainTab(k)} style={{ background: mainTab===k ? `${accentColor}22` : "#0c0c18", border: `1px solid ${mainTab===k ? accentColor : "#2a2a3a"}`, borderRadius: 6, color: mainTab===k ? accentColor : "#888", padding: "9px 24px", cursor: "pointer", fontFamily: "monospace", fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", transition: "all 0.2s" }}>{l}</button>
              ))}
            </div>
          </div>
          ...
          {mainTab === "saved" && (
            <div style={{ maxWidth: 620, margin: "0 auto", animation: "fadeUp 0.3s ease" }}>
              {saved.length === 0
                ? <div style={{ textAlign: "center", color: "#555", fontSize: 14, padding: "70px 0", fontFamily: "monospace" }}>No saved content yet!</div>
                : saved.map((p, i) => {
                    const pct = CONTENT_TYPES[p.contentType];
                    return (
                      <div key={i} onClick={() => loadPost(p)} style={{ background: "#0c0c18", border: "1px solid #252535", borderRadius: 9, padding: "14px 18px", cursor: "pointer", marginBottom: 10, display: "flex", gap: 14, alignItems: "center", transition: "border-color 0.2s" }}
                      ...
                      >
                        ...
                        <div style={{ flex: 1, overflow: "hidden" }}>
                          <div style={{ color: "#666", fontSize: 10, fontFamily: "monospace", marginBottom: 4 }}>{pct.label.toUpperCase()} · {p.aura.toUpperCase()}</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <Card>
                  ...
                  {Object.entries(CONTENT_TYPES).map(([k,v]) => (
                    <div key={k} onClick={() => setContentType(k)} style={{ background: contentType===k ? `${v.color}18` : "#0a0a15", border: `1px solid ${contentType===k ? v.color : "#222"}`, borderRadius: 7, padding: "11px 15px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, marginBottom: 8, transition: "all 0.2s", boxShadow: contentType===k ? `0 0 16px ${v.color}44` : "none" }}>
                      <span style={{ fontSize: 22 }}>{v.icon}</span>
                      <div style={{ color: contentType===k ? v.color : "#ccc", fontSize: 13, fontFamily: "monospace", fontWeight: 700 }}>{v.label}</div>
                    </div>
                  ))}
                </Card>
                ...
                <Card>
                  ...
                  {Object.entries(AURAS).map(([k,v]) => (
                    <div key={k} onClick={() => setAura(k)} style={{ background: aura===k ? `${v.color}15` : "#0a0a15", border: `1px solid ${aura===k ? v.color : "#222"}`, borderRadius: 7, padding: "11px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, marginBottom: 8, transition: "all 0.2s", boxShadow: aura===k ? v.glow : "none" }}>
                      <span style={{ fontSize: 22 }}>{v.icon}</span>
                      <div>
                        <div style={{ color: aura===k ? v.color : "#ccc", fontSize: 13, fontFamily: "monospace", fontWeight: 700 }}>{v.label}</div>
                ...
                <Card>
                  ...
                  {Object.entries(DESIGNS).map(([k,v]) => (
                    <div key={k} onClick={() => setDesign(k)} style={{ background: design===k ? `${v.accent}18` : "#0a0a15", border: `1px solid ${design===k ? v.accent : "#222"}`, borderRadius: 6, padding: "11px 14px", cursor: "pointer", marginBottom: 7, color: design===k ? v.accent : "#bbb", fontSize: 13, fontFamily: "monospace", fontWeight: design===k ? 700 : 400, transition: "all 0.2s", boxShadow: design===k ? `0 0 12px ${v.accent}44` : "none" }}>{v.label}</div>
                  ))}
                </Card>
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {QUICK_TOPICS[aura].map((t,i) => (
                        <div key={i} onClick={() => setTopic(t)} style={{ background: "#0f0f20", border: `1px solid ${accentColor}40`, borderRadius: 5, color: accentColor, padding: "5px 10px", fontSize: 11, cursor: "pointer", fontFamily: "monospace", transition: "background 0.15s" }}
                ...
                <button onClick={generate} disabled={writing || !topic.trim()} style={{ background: writing || !topic.trim() ? "#0a0a15" : `${accentColor}20`, border: `2px solid ${writing || !topic.trim() ? "#333" : accentColor}`, borderRadius: 8, color: !topic.trim() ? "#555" : accentColor, padding: "17px 0", cursor: writing || !topic.trim() ? "not-allowed" : "pointer", fontFamily: "'Orbitron',monospace", fontSize: 12, letterSpacing: "0.2em", fontWeight: 900, boxShadow: writing || !topic.trim() ? "none" : `0 0 20px ${accentColor}55`, transition: "all 0.3s", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                  {writing
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                {contentType === "instagram" && tab === "preview" && (
                  <div style={{ animation: "fadeUp 0.3s ease", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, alignItems: "start" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      {result?.seoTitle && <Card><SLabel>◈  SEO TITLE</SLabel><div style={{ color: dc.accent, fontSize: 15, fontFamily: "monospace", fontWeight: 700, lineHeight: 1.5, marginBottom: 12 }}>{result.seoTitle}</div><CopyBtn text={result.seoTitle} color={dc.accent} /></Card>}
                      {result && <Card><SLabel>◈  STATS</SLabel>{[{label:"Caption words",value:result.caption?.split(/\s+/).length??0,color:ac.color},{label:"Hashtags",value:result.hashtags?.length??0,color:ac.color},{label:"SEO keywords",value:result.seoKeywords?.length??0,color:dc.accent}].map(({label,value,color},i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:i<2?"1px solid #1a1a28":"none"}}><span style={{fontSize:12,color:"#bbb",fontFamily:"monospace"}}>{label}</span><span style={{fontSize:14,color,fontFamily:"monospace",fontWeight:700}}>{value}</span></div>)}</Card>}
                      {result && <ExportCard items={[{label:"Caption only",text:result.caption},{label:"Hashtags only",text:"#"+result.hashtags?.join(" #")},{label:"Caption + Hashtags",text:fullIGText},{label:"Alt Text",text:result.altText},{label:"SEO Keywords",text:result.seoKeywords?.join(", ")}]} />}
                ...
                {contentType === "instagram" && tab === "caption" && (
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    ...
                    <TextBlock label="◈  ALT TEXT" content={result?.altText} color="#aaa" />
                    {!result && <EmptyState color={ac.color} />}
                  </div>
                )}
                  ...
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    {result?.hashtags ? <>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 4 }}>
                        {result.hashtags.map((h,i) => <span key={i} style={{ background:`${ac.color}14`,border:`1px solid ${ac.color}44`,borderRadius:5,color:ac.color,padding:"6px 11px",fontSize:12,fontFamily:"monospace" }}>#{h}</span>)}
                      </div>
                ...
                {contentType === "blog" && tab === "preview" && (
                  <div style={{ animation: "fadeUp 0.3s ease", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, alignItems: "start" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      {result && <Card><SLabel>◈  BLOG STATS</SLabel>{[{label:"Read time",value:result.readTime||"—",color:ac.color},{label:"SEO keywords",value:result.seoKeywords?.length??0,color:dc.accent},{label:"Sections",value:result.blogOutline?.length??0,color:ac.color}].map(({label,value,color},i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:i<2?"1px solid #1a1a28":"none"}}><span style={{fontSize:12,color:"#bbb",fontFamily:"monospace"}}>{label}</span><span style={{fontSize:13,color,fontFamily:"monospace",fontWeight:700}}>{value}</span></div>)}</Card>}
                      {result && <ExportCard items={[{label:"Blog title",text:result.blogTitle},{label:"Meta description",text:result.metaDescription},{label:"Full article",text:result.blogBody},{label:"SEO keywords",text:result.seoKeywords?.join(", ")}]} />}
                      {!result && <EmptyState color={ac.color} />}
                    </div>
                  </div>
                )}
                {contentType === "blog" && tab === "article" && (
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    ...
                    <TextBlock label="◈  FULL ARTICLE" content={result?.blogBody} color={ac.color} />
                    {!result && <EmptyState color={ac.color} />}
                  </div>
                )}
                {contentType === "blog" && tab === "outline" && (
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    {result?.blogOutline ? <>
                      <TextBlock label="◈  ARTICLE OUTLINE" content={result.blogOutline.map((h,i)=>`${i+1}. ${h}`).join("\n")} color={ac.color} />
                      {result.blogOutline.map((h,i) => (
                        <div key={i} style={{ background: "#10101a", border:`1px solid ${ac.color}25`, borderRadius:6, padding:"12px 16px", display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                      ...
                      {result.internalLinks && <div style={{marginTop:16}}><TextBlock label="◈  SUGGESTED INTERNAL LINKS" content={result.internalLinks.join("\n")} color={dc.accent} /></div>}
                    </> : <EmptyState color={ac.color} />}
                  </div>
                )}
                {contentType === "blog" && tab === "seo" && (
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    ...
                    <TextBlock label="◈  TARGET KEYWORDS" content={result?.seoKeywords?.join(", ")} color={ac.color} />
                    <TextBlock label="◈  META TAGS" content={result?.metaTags?.join(", ")} color="#aaa" />
                    {!result && <EmptyState color={ac.color} />}
                  </div>
                )}
                ...
                {contentType === "email" && tab === "preview" && (
                  <div style={{ animation: "fadeUp 0.3s ease", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, alignItems: "start" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      {result && <Card><SLabel>◈  EMAIL STATS</SLabel>{[{label:"Subject chars",value:result.emailSubject?.length??0,color:ac.color},{label:"Preheader chars",value:result.preheader?.length??0,color:dc.accent},{label:"Body words",value:result.emailBody?.split(/\s+/).length??0,color:ac.color}].map(({label,value,color},i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:i<2?"1px solid #1a1a28":"none"}}><span style={{fontSize:12,color:"#bbb",fontFamily:"monospace"}}>{label}</span><span style={{fontSize:13,color,fontFamily:"monospace",fontWeight:700}}>{value}</span></div>)}</Card>}
                      {result && <ExportCard items={[{label:"Subject line",text:result.emailSubject},{label:"Preheader",text:result.preheader},{label:"Email body",text:result.emailBody},{label:"CTA button",text:result.ctaText}]} />}
                      {!result && <EmptyState color={ac.color} />}
                    </div>
                  </div>
                )}
                {contentType === "email" && tab === "copy" && (
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    ...
                    <TextBlock label="◈  SEGMENT TIP" content={result?.segmentTip} color="#aaa" />
                    {!result && <EmptyState color={ac.color} />}
                  </div>
                )}
                  ...
                  <div style={{ animation: "fadeUp 0.3s ease" }}>
                    {result?.subjectLineVariants ? <>
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize:11, color:"#888", letterSpacing:"0.2em", fontFamily:"monospace", fontWeight:700, marginBottom:12 }}>◈  SUBJECT LINE VARIANTS — A/B TEST</div>
                    ...
                    {result?.imagePrompts ? <>
                      <div style={{ background:"#0c0c18", border:`1px solid ${dc.accent}33`, borderRadius:8, padding:"14px 18px", marginBottom:16 }}>
                        <div style={{ fontSize:12, color:"#bbb", fontFamily:"monospace", lineHeight:1.7 }}>
                          🎨 <strong style={{color:dc.accent}}>4 unique image prompts</strong> — paste into <strong style={{color:"#fff"}}>Midjourney</strong>, <strong style={{color:"#fff"}}>DALL·E 3</strong>, or <strong style={{color:"#fff"}}>Adobe Firefly</strong>. Each is a presentation-style visual with text overlay.
                        </div>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                          ...
                          <div key={i} style={{ background:"#10101a", border:`1px solid ${dc.accent}30`, borderLeft:`3px solid ${dc.accent}`, borderRadius:6, padding:"16px 18px" }}>
                            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                                <span style={{ background:`${dc.accent}22`, border:`1px solid ${dc.accent}55`, borderRadius:4, color:dc.accent, padding:"3px 10px", fontSize:10, fontFamily:"monospace", letterSpacing:"0.15em", fontWeight:700 }}>{["HERO","CAROUSEL","STORY","MINIMAL"][i]}</span>
                                <span style={{ color:"#888", fontSize:12, fontFamily:"monospace" }}>{p.label}</span>
                              </div>
                              <CopyBtn text={p.prompt} color={dc.accent} />
                            </div>
                            <div style={{ color:"#d0d0d0", fontSize:13, lineHeight:1.8, fontFamily:"monospace" }}>{p.prompt}</div>
                          </div>
                        ))}
                      </div>
                    </> : <EmptyState color={ac.color} />}
